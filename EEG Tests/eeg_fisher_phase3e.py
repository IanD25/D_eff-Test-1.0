...
Downloads CHB-MIT EDF data automatically. Results saved to phase3e_results/.
"""
...
def download_patient_data(patient):
    """Download seizure EDF files for one patient from PhysioNet."""
    ...
    try:
        ...
        # Download each seizure EDF file directly
        for sz in seizures:
            edf_name = sz["file"]
...
def load_eeg_segment(patient_dir, filename, start_sec, duration_sec):
    ...
    try:
        ...
        if start_sample >= end_sample:
            return None, None
        data = raw.get_data(start=start_sample, stop=end_sample)  # (n_channels, n_samples)
